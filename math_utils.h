/*
    this header gives me all the basic vector and matrix tools i
    need for this assignment. i do not have to write this math
    from scratch, but i should understand at a high level what i
    am using:

    - vector3f: stores a 3d vector (x, y, z) and supports length,
      normalization, addition, subtraction, scaling, cross product.

    - matrix4f: stores a 4x4 matrix and has helpers to build common
      transforms such as rotation, translation, scaling, perspective.

    in my game code i will mostly call methods like:
      - matrix4f scale, rotate, translate
      - combine them into a single transform for each object
      - send the final matrix to the vertex shader as gworld

    if i can look at one of these matrices and roughly explain what
    it is doing (e.g. “this rotates 30 degrees around z and then
    moves the object to (x, y, z)”), i am in a good place for this
    course.

    the original implementation is by etay meiri and is licensed
    under gpl, as noted below.
*/
#ifndef MATH_UTILS_H
#define MATH_UTILS_H
#include <math.h>
